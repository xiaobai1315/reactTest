...
function deepStrictEqualValues(buf, arr) {
  ...
  try {
    for (var _iterator = buf.entries()[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
...
deepStrictEqualValues(genBuffer(4, [hexBufFill, 1, -1]), [0, 0, 0, 0]);
assert.throws(function () {
  return buf1.fill(0, -1);
});
assert.throws(function () {
  return buf1.fill(0, 0, buf1.length + 1);
});
assert.throws(function () {
  return buf1.fill('', -1);
});
assert.throws(function () {
  return buf1.fill('', 0, buf1.length + 1);
});
assert.throws(function () {
  return buf1.fill('a', 0, buf1.length, 'node rocks!');
});
assert.throws(function () {
  return buf1.fill('a', 0, 0, NaN);
});
assert.throws(function () {
  return buf1.fill('a', 0, 0, null);
});
assert.throws(function () {
  return buf1.fill('a', 0, 0, 'foo');
});
...
function writeToFill(string, offset, end, encoding) {
  ...
  if (offset < 0 || end > buf2.length) throw new RangeError('Out of range index');
  ...
  do {
    ...
    offset += written;
    if (written === 0) {
      if (wasZero) throw new Error('Could not write all data to Buffer');else wasZero = true;
    }
...
//# sourceMappingURL=test-buffer-fill.js.map