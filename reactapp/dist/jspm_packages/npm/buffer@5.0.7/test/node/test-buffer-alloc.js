...
assert.throws(function () {
  return Buffer.alloc(-Buffer.poolSize);
}, '"size" argument must not be negative');
assert.throws(function () {
  return Buffer.alloc(-100);
}, '"size" argument must not be negative');
assert.throws(function () {
  return Buffer.allocUnsafe(-Buffer.poolSize);
}, '"size" argument must not be negative');
assert.throws(function () {
  return Buffer.allocUnsafe(-100);
}, '"size" argument must not be negative');
assert.throws(function () {
  return Buffer.allocUnsafeSlow(-Buffer.poolSize);
}, '"size" argument must not be negative');
assert.throws(function () {
  return Buffer.allocUnsafeSlow(-100);
}, '"size" argument must not be negative');
assert.throws(function () {
  return Buffer.alloc({ valueOf: function valueOf() {
      return 1;
    } });
...
//# sourceMappingURL=test-buffer-alloc.js.map